...
import (
	...
	"strconv"
	"tomatoscan/scanner/portScan"
)

func PortScanBox() *fyne.Container {
	//  输入框和其绑定的列表
	ipBox, ipList := inputList("Input Hosts")
	portBox, portList := inputList("Input Ports")
	//  装输入框的容器
	...
	speedList := []string{"2000", "3000", "4000", "5000"}
	...
	if portScan.AllHostsNum == 0 {
		progress.Value = float64(0)
	} else {
		progress.Value = float64(portScan.AccomplishHostsNum) / float64(portScan.AllHostsNum)
	}
	//  结束标志
	var endFlag bool
	//  开始按钮
	startPortScan := widget.NewButton("Run", func() {
		...
		//  开始扫描
		go func() {
			addressList, _, _, _ := portScan.PortScan(ips, append(ports, "-1"), speed)
